...
d3.json("limits.php", function(error, json) {
    ...
    if (data.length != 0) {
        ...
        $('#datatable th:nth-child(5)').append("<a href=\"#\" class=\"tooltip\">*<span><img class=\"callout\" src=\"/images/icons/callout.gif\" />" +
...
function fill_table(table_container) {
    ...
    var columns = ["User", "Jobs", "CPU cores", "Nodes", "Job Wall Time", "CPU-Minutes"];
    var columns_json = ["user", "grp_submit_jobs", "grp_cpus", "grp_nodes", "grp_wall", "grp_cpu_run_mins"];
...
function updateCommand() {
    var limit_types = ["grp_submit_jobs", "grp_cpus", "grp_nodes", "grp_wall", "grp_cpu_run_mins"];
    var limit_types_slurm = ["GrpJobs", "GrpCPUs", "GrpNodes", "MaxWall", "GrpCPURunMins"];

    // detect differences between the data and the original data
    var update_string = "";
    for (i=0; i < data.length; i++) {
        for (t=0; t < limit_types.length; t++) {
            if (original_data[i][limit_types[t]] != data[i][limit_types[t]]){
